...
<!-- I changed the tense to present tense, since this is the description of the general procedure -->
...
RPS::plot_RPS(sampH0 = sampH0, sampH1 = sampH1, sampEmp = sampleEmp, select = 1)
```

One can use the **Figure functions** separately,

```{r}
sampH1 <- RPS::samplesH1(Nsample = 100, alpha = 0.05, effectSize = 0.1, pow = 0.95)
...
##########################################################
#the figure function has bugs, and doesn't work
...
##########################################################################
# can we split the table in two, so that the false negative are separately present maybe add another line in between
# Also maybe you can add the substantial verification with larger N as
# 6.1 Substantial Verification with larger N (N' = 1.5 * N = x, 1 - beta (Greek letter) = )
